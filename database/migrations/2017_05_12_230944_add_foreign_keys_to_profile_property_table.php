...
class AddForeignKeysToProfilePropertyTable extends Migration {
	...
	public function up()
	{
		Schema::table('profile_property', function(Blueprint $table)
		{
			...
			$table->foreign('updated_by', 'profile_property_ibfk_4')->references('id')->on('users')->onUpdate('NO ACTION')->onDelete('SET NULL');
			$table->foreign('created_by', 'profile_property_ibfk_5')->references('id')->on('users')->onUpdate('NO ACTION')->onDelete('SET NULL');
			$table->foreign('deleted_by', 'profile_property_ibfk_6')->references('id')->on('users')->onUpdate('NO ACTION')->onDelete('SET NULL');
		});
	}
