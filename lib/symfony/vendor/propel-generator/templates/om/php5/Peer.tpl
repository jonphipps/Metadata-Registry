...
<?php
	foreach ($table->getColumns() as $col) {
		if (!$col->isLazyLoad()) {
	?>
    $criteria->addSelectColumn(($tableAlias) ? self::alias($tableAlias, self::<?php echo PeerBuilder::getColumnName($col) ?>) : self::<?php echo PeerBuilder::getColumnName($col) ?>);
<?php
		}
	} ?>
	}
    ...
    $tableAliasIndex = 1;
		foreach ($table->getForeignKeys() as $fk) {
			...
			if ( $fk->getForeignTableName() != $table->getName() ) {
				...
				$new_index = $index + 1;
?>
		<?php echo $joinClassName ?>Peer::addSelectColumns($c, '<?php echo 'a' . $tableAliasIndex ?>');
		...
		foreach ($table->getForeignKeys() as $fk) {
			...
			if ( $fk->getForeignTableName() != $table->getName() ) {
				...
				foreach ($fk->getLocalColumns() as $columnName ) {
					...
					$columnFk = $joinTable->getColumn( $lfMap[$columnName]);
?>
    $c->addJoin(<?php echo PeerBuilder::getColumnName($column, $table->getPhpName()) ?>, <?php echo $table->getPhpName() ?>::alias('<?php echo 'a' . $tableAliasIndex ?>', <?php echo PeerBuilder::getColumnName($columnFk,$joinClassName)?>));
    $c->addAlias('<?php echo 'a' . $tableAliasIndex ?>', <?php echo $table->getPhpName() ?>::TABLE_NAME);
	 <?php  } } $tableAliasIndex++; }?>
