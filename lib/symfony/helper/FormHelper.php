...
function options_for_select( $options = array (), $selected = '', $html_options = array () )
{
  ...
  foreach ( $options as $key => $value ) {
    if ( is_array( $value ) || $value instanceof sfOutputEscaperArrayDecorator ) {
      $html .=
          content_tag( 'optgroup', options_for_select( $value, $selected, $html_options ), array ( 'label' => $key ) )
          . "\n";
    } else {
...
function input_date_range_tag( $name, $value, $options = array () )
{
  ...
  return $before . input_date_tag( $name . '[from]', isset( $value[ 'from' ] ) ? $value[ 'from' ] : null, $options )
         . $middle . input_date_tag(
             $name . '[to]',
             isset( $value[ 'to' ] ) ? $value[ 'to' ] : null,
             $options
         ) . $after;
}
...
function input_date_tag( $name, $value = null, $options = array () )
{
  ...
  if ( ! _get_option( $options, 'rich', false ) ) {
    ...
    if ( $withTime ) {
      return select_datetime_tag(
          $name,
          $value,
          $options,
          isset( $options[ 'html' ] ) ? $options[ 'html' ] : array ()
      );
    } else {
