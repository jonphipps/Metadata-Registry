...
sk:releaseChanges           2004-07-21: release 0.1.0
                            ...
                            2005-07-31: revision 0.2.4
                                        - added a trim()s to literal data
...
class ARC_rdfxml_parser{
  ...
  function init($create_parser=true){
    ...
    /* save_data */
    if($this->init_args["save_data"]){
      $this->save_data=true;
      $this->data="";
    }
    /* max_lines */
    if($this->init_args["max_lines"]){
      $this->max_lines=$this->init_args["max_lines"];
    }
    /* encoding */
    if(isset($this->init_args["encoding"])){
  ...
  function set_base($base){
    if(strlen($this->xml_base===0) || (strpos($path, ":")!==false)){
      $this->xml_base=$base;
    }
    else{
  ...
  function get_cur_xml_base($s=""){
    if($s){
      if($base=$s["p_xml_base"]){
        return $base;
      }
      elseif($base=$s["xml_base"]){
        return $base;
      }
    }
  ...
  function get_cur_lang($s=""){
    if($s){
      if($lang=$s["p_xml_lang"]){
        return $lang;
      }
      elseif($lang=$s["xml_lang"]){
        return $lang;
      }
    }
  ...
  function handle_open_2($tag, $attrs){
    ...
    /* adjust li */
    if($cur_p==="http://www.w3.org/1999/02/22-rdf-syntax-ns# li"){
      $li_count=$cur_s["li_count"]+1;
      $cur_s["li_count"]=$li_count;
      $cur_p="http://www.w3.org/1999/02/22-rdf-syntax-ns#_".$li_count;
    }
    ...
    /* rdf:resource */
    if(isset($attrs["http://www.w3.org/1999/02/22-rdf-syntax-ns# resource"])){
      $rdf_resource=$attrs["http://www.w3.org/1999/02/22-rdf-syntax-ns# resource"];
      $rdf_resource=$this->calc_uri($cur_s, $rdf_resource, "resource");
      $this->add_triple($cur_s, $cur_s["cur_p"], array("type"=>"uri", "uri"=>$rdf_resource));
      /* typing */
      if(isset($attrs["http://www.w3.org/1999/02/22-rdf-syntax-ns# type"])){
        $this->add_triple(array("type"=>"uri", "uri"=>$rdf_resource), "http://www.w3.org/1999/02/22-rdf-syntax-ns#type", array("type"=>"uri", "uri"=>$attrs["http://www.w3.org/1999/02/22-rdf-syntax-ns# type"]));
      }
      /* reification */
    ...
    /* named bnode */
    elseif($rdf_nodeID=$attrs["http://www.w3.org/1999/02/22-rdf-syntax-ns# nodeID"]){
    ...
    /* rdf:parseType */
    elseif($rdf_parseType=$attrs["http://www.w3.org/1999/02/22-rdf-syntax-ns# parseType"]){
      ...
      elseif($rdf_parseType==="Resource"){
        ...
        $this->push_s($sub_s);
        if($p_rdf_ID=$cur_s["p_rdf_ID"]){/* reify, p is an empty element */
      ...
      /* typed literal */
      if($rdf_datatype=$attrs["http://www.w3.org/1999/02/22-rdf-syntax-ns# datatype"]){
    ...
    unset($tmp_node);
    foreach($attrs as $k=>$v){
      if(strpos($k, "http://www.w3.org/1999/02/22-rdf-syntax-ns#")===false && strpos($k, "http://www.w3.org/XML/1998/namespace")===false && strpos($k, " ")!==false){
        if(!$tmp_node){
          $cur_lang=$this->get_cur_lang($cur_s);
          if($rdf_resource){
        ...
        if($p_rdf_ID=$cur_s["p_rdf_ID"]){/* reify, but only once, p is an empty element */
    ...
    switch($this->state){
      ...
      case 4:/* empty p or p_close after cdata reading or p_close after collection */
        ...
        if($cur_s["is_list"]){
          ...
          $this->add_triple($cur_s, $cur_s["cur_p"], array("type"=>"literal", "val"=>trim($cur_s["o_cdata"]), "dt"=>$cur_s["o_rdf_datatype"], "lang"=>$this->get_cur_lang($cur_s)));
          if($p_rdf_ID=$cur_s["p_rdf_ID"]){/* reify */
            $this->reify(array("type"=>"uri", "uri"=>$this->calc_uri($cur_s, $p_rdf_ID, "ID")), $cur_s, array("type"=>"uri", "uri"=>$cur_s["cur_p"]), array("type"=>"literal", "val"=>trim($cur_s["o_cdata"]), "dt"=>$cur_s["o_rdf_datatype"], "lang"=>$this->get_cur_lang($cur_s)));
          }
          unset($cur_s["o_cdata"]);
