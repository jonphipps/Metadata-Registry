...
version:  2008-08-04 (Addition: ARC2_JSONParser
                      ...
                      Additions in ARC2_Reader
                      Addition in ARC2_Class
                      Additions in ARC2_RDFParser
                      Addition in ARC2_StoreLoadQueryHandler
                      Additions in ARC2_TurtleParser
                      Addition in ARC2_SPARQLParser
                      Addition in ARC2_SemHTMLParser
                      Additions in ARC2_Reader
                      Addition in ARC2_SPARQLScriptParser
                      Additions in ARC2_SPARQLScriptProcessor
                      Addition in ARC2_Store
                      Fix in ARC2_SPARQLScriptParser
                      Fix in ARC2_StoreEndpoint
                      Fix in ARC2_NTriplesSerializer
                      Fix in ARC2_TurtleSerializer
                      Fix in ARC2_LegacyXMLParser
                      Tweak: using in_array for triple dupe checks in getSimpleIndex (thx to mmmmrob)
                      Tweak in ARC2_Reader
                      Tweaks in ARC2_StoreDeleteQueryHandler
                      Tweak in ARC2_RDFXMLParser
                      Tweak in ARC2_SGAJSONParser
...
class ARC2 {
  ...
  public static function getIncPath($f = '') {
    $r = realpath(dirname(__FILE__)) . DIRECTORY_SEPARATOR;
    $dirs = array(
      'plugin' => 'plugins',
      'trigger' => 'triggers',
      'store' => 'store',
    ...
    foreach ($dirs as $k => $dir) {
      if (preg_match('/' . $k . '/i', $f)) {
        return $r .= $dir . DIRECTORY_SEPARATOR;
      }
    }
    return $r;
  }
  ...
  public static function inc($f) {
    ...
    if ($prefix != 'ARC2') {
      $path = $inc_path . strtolower($prefix) . DIRECTORY_SEPARATOR . $prefix . '_' . urlencode($f) . '.php';
      if (file_exists($path)) {
        include_once($path);
        return 1;
      }
  ...
  public static function getComponent($name, $a = '') {
    ARC2::inc($name);
    $prefix = 'ARC2';
  ...
  public static function getStore($a = '') {
