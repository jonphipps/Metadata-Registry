...
class pakeApp
{
  ...
  public function run($pakefile = null, $options = null)
  {
    ...
    else
    {
      ...
      {
        throw new pakeException('task "'.$task.'" is not defined');
      }
      else if (count($abbrev_options[$task]) > 1)
      {
        throw new pakeException('task "'.$task.'" is ambiguous ('.implode(', ', $abbrev_options[$task]).')');
      }
      else
      {
  ...
  public function load_pakefile()
  {
    ...
    while (!$this->have_pakefile())
    {
      ...
      {
        throw new pakeException('no pakefile found (looking for: '.join(', ', pakeApp::$PAKEFILES).')'."\n");
      }
