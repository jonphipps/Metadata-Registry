...
abstract class BaseRoleUser extends BaseObject  implements Persistent {
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			return $startcol + 3; // 3 = RoleUserPeer::NUM_COLUMNS - RoleUserPeer::NUM_LAZY_LOAD_COLUMNS).
    ...
    foreach (sfMixer::getCallables('BaseRoleUser:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseRoleUser:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseRoleUser:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseRoleUser:save:post') as $callable)
    {
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseRoleUser:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseRoleUser::%s', $method));
    }
