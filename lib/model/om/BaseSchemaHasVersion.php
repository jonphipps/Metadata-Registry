...
abstract class BaseSchemaHasVersion extends BaseObject  implements Persistent {
  ...
  const PEER = 'SchemaHasVersionPeer';
	...
	/**
	 * Initializes internal state of BaseSchemaHasVersion object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [timeslice] column value.
	 * 
	 *
	 * @param      string $format The date/time format string (either date()-style or strftime()-style).
	 *							If format is NULL, then the raw DateTime object will be returned.
	...
	public function getTimeslice($format = 'Y-m-d H:i:s')
	{
		...
		if ($this->timeslice === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->timeslice);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->timeslice, true), $x);
			}
		}
	 ...
	 * @return     SchemaHasVersion The current object (for fluent API support)
	 */
	...
	/**
	 * Sets the value of [timeslice] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.  Empty string will
	 *						be treated as NULL for temporal objects.
	 * @return     SchemaHasVersion The current object (for fluent API support)
	...
	public function setTimeslice($v)
	{
		...
		if ( $this->timeslice !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->timeslice !== null && $tmpDt = new DateTime($this->timeslice)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->timeslice = ($dt ? $dt->format('Y-m-d H:i:s') : null);
	...
	public function hasOnlyDefaultValues()
	{
			...
			if (array_diff($this->modifiedColumns, array(SchemaHasVersionPeer::NAME))) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->timeslice = ($row[$startcol + 7] !== null) ? (string) $row[$startcol + 7] : null;
			$this->resetModified();
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = SchemaHasVersionPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseSchemaHasVersion:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
			...
			SchemaHasVersionPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	public function getUser(PropelPDO $con = null)
	{
		if ($this->aUser === null && ($this->created_user_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aUser->addSchemaHasVersions($this);
			 */
		}
		return $this->aUser;
	}
	...
	public function getSchema(PropelPDO $con = null)
	{
		if ($this->aSchema === null && ($this->schema_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aSchema->addSchemaHasVersions($this);
			 */
		}
		return $this->aSchema;
	}
