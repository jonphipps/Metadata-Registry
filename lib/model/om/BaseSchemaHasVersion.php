...
/**
 * Base class that represents a row from the 'schema_has_version' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BaseSchemaHasVersion extends BaseObject  implements Persistent {
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = SchemaHasVersionPeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v || $v === '') {
			$this->name = $v;
			$this->modifiedColumns[] = SchemaHasVersionPeer::NAME;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ($this->created_at !== $ts) {
			$this->created_at = $ts;
			$this->modifiedColumns[] = SchemaHasVersionPeer::CREATED_AT;
		}
	...
	public function setUpdatedAt($v)
	{
		...
		if ($this->updated_at !== $ts) {
			$this->updated_at = $ts;
			$this->modifiedColumns[] = SchemaHasVersionPeer::UPDATED_AT;
		}
	...
	public function setSchemaId($v)
	{
		...
		if ($this->schema_id !== $v) {
			$this->schema_id = $v;
			$this->modifiedColumns[] = SchemaHasVersionPeer::SCHEMA_ID;
		}
	...
	public function setTimeslice($v)
	{
		...
		if ($this->timeslice !== $ts) {
			$this->timeslice = $ts;
			$this->modifiedColumns[] = SchemaHasVersionPeer::TIMESLICE;
		}
    ...
    foreach (sfMixer::getCallables('BaseSchemaHasVersion:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			$con->begin();
			SchemaHasVersionPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseSchemaHasVersion:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseSchemaHasVersion:save:pre') as $callable)
    {
    ...
    if ($this->isNew() && !$this->isColumnModified(SchemaHasVersionPeer::CREATED_AT))
    {
      $this->setCreatedAt(time());
    }

    if ($this->isModified() && !$this->isColumnModified(SchemaHasVersionPeer::UPDATED_AT))
    {
      $this->setUpdatedAt(time());
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseSchemaHasVersion:save:post') as $callable)
    {
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = SchemaHasVersionPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = SchemaHasVersionPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = SchemaHasVersionPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(SchemaHasVersionPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of SchemaHasVersion (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     SchemaHasVersion Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     SchemaHasVersionPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new SchemaHasVersionPeer();
		}
		return self::$peer;
	}
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseSchemaHasVersion:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseSchemaHasVersion::%s', $method));
    }
