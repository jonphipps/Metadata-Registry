...
/**
 * Base class that represents a row from the 'arc_s2val' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BaseArcS2val extends BaseObject  implements Persistent {
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = ArcS2valPeer::ID;
		}
	...
	public function setCid($v)
	{
		...
		if ($this->cid !== $v || $v === 0) {
			$this->cid = $v;
			$this->modifiedColumns[] = ArcS2valPeer::CID;
		}
	...
	public function setMisc($v)
	{
		...
		if ($this->misc !== $v || $v === false) {
			$this->misc = $v;
			$this->modifiedColumns[] = ArcS2valPeer::MISC;
		}
	...
	public function setVal($v)
	{
		...
		if ($this->val !== $v) {
			$this->val = $v;
			$this->modifiedColumns[] = ArcS2valPeer::VAL;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			return $startcol + 4; // 4 = ArcS2valPeer::NUM_COLUMNS - ArcS2valPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating ArcS2val object", $e);
		}
	}
    ...
    foreach (sfMixer::getCallables('BaseArcS2val:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			$con->begin();
			ArcS2valPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseArcS2val:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseArcS2val:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseArcS2val:save:post') as $callable)
    {
	...
	protected function doSave($con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = ArcS2valPeer::doInsert($this, $con);
					...
					$affectedRows += ArcS2valPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = ArcS2valPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = ArcS2valPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = ArcS2valPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getCid(),
			$keys[2] => $this->getMisc(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ArcS2valPeer::ID)) $criteria->add(ArcS2valPeer::ID, $this->id);
		if ($this->isColumnModified(ArcS2valPeer::CID)) $criteria->add(ArcS2valPeer::CID, $this->cid);
		if ($this->isColumnModified(ArcS2valPeer::MISC)) $criteria->add(ArcS2valPeer::MISC, $this->misc);
		if ($this->isColumnModified(ArcS2valPeer::VAL)) $criteria->add(ArcS2valPeer::VAL, $this->val);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(ArcS2valPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of ArcS2val (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     ArcS2val Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * Otherwise if this ArcS2val has previously
	 * been saved, it will retrieve related ArcTriples from storage.
	 * If this ArcS2val is new, it will return
		...
		if ($this->collArcTriples === null) {
			if ($this->isNew()) {
			   $this->collArcTriples = array();
				...
				$criteria->add(ArcTriplePeer::S, $this->getId());
	 ...
	 * If this collection has already been initialized with
	 * an identical criteria, it returns the collection.
	 * Otherwise if this ArcS2val is new, it will return
	 * an empty collection; or if this ArcS2val has previously
	 * been saved, it will retrieve related ArcTriples from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in ArcS2val.
	 */
	 ...
	 * actually need in ArcS2val.
	 */
	public function getArcTriplesJoinArcO2val($criteria = null, $con = null)
	{
		// include the Peer class
		include_once 'lib/model/om/BaseArcTriplePeer.php';
		if ($criteria === null) {
		...
		if ($this->collArcTriples === null) {
			if ($this->isNew()) {
				...
				$this->collArcTriples = ArcTriplePeer::doSelectJoinArcO2val($criteria, $con);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseArcS2val:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseArcS2val::%s', $method));
    }
