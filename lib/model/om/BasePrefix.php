...
/**
 * Base class that represents a row from the 'reg_prefix' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BasePrefix extends BaseObject  implements Persistent {
	...
	public function setUri($v)
	{
		...
		if ($this->uri !== $v) {
			$this->uri = $v;
			$this->modifiedColumns[] = PrefixPeer::URI;
		}
	...
	public function setRank($v)
	{
		...
		if ($this->rank !== $v || $v === 0) {
			$this->rank = $v;
			$this->modifiedColumns[] = PrefixPeer::RANK;
		}
    ...
    foreach (sfMixer::getCallables('BasePrefix:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePrefix:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePrefix:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BasePrefix:save:post') as $callable)
    {
	...
	protected function doSave($con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = PrefixPeer::doInsert($this, $con);
					...
					$affectedRows += PrefixPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = PrefixPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = PrefixPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Prefix (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Prefix Clone of current object.
	 * @throws     PropelException
	 */
  ...
  {
    if (!$callable = sfMixer::getCallable('BasePrefix:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BasePrefix::%s', $method));
    }
