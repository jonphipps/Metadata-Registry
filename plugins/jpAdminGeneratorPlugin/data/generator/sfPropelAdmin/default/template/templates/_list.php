...
if ($parents): ?>
  ...
  <?php endforeach; ?>
<?php endif; ?>
  [?php echo link_to(image_tag(sfConfig::get('sf_admin_web_dir').'/images/first.png', array('alt' => __('First'), 'title' => __('First'))), '@' . $parent . '<?php echo $this->getModuleName() ?>_list?' .$filterParam, ['query_string' => '&page=1']) ?]
  [?php echo link_to(image_tag(sfConfig::get('sf_admin_web_dir').'/images/previous.png', array('alt' => __('Previous'), 'title' => __('Previous'))), '@' . $parent . '<?php echo $this->getModuleName() ?>_list?' .$filterParam, ['query_string' => '&page='.$pager->getPreviousPage()]) ?]

  [?php foreach ($pager->getLinks() as $page): ?]
    [?php echo link_to_unless($page == $pager->getPage(), $page, '@' . $parent . '<?php echo $this->getModuleName() ?>_list?' . $filterParam, ['query_string' => '&page=' . $page]) ?]
  [?php endforeach; ?]

  [?php echo link_to(image_tag(sfConfig::get('sf_admin_web_dir').'/images/next.png', array('alt' => __('Next'), 'title' => __('Next'))), '@' . $parent . '<?php echo $this->getModuleName() ?>_list?' .$filterParam, ['query_string' => '&page='.$pager->getNextPage()]) ?]
  [?php echo link_to(image_tag(sfConfig::get('sf_admin_web_dir').'/images/last.png', array('alt' => __('Last'), 'title' => __('Last'))), '@' . $parent . '<?php echo $this->getModuleName() ?>_list?' .$filterParam, ['query_string' => '&page='.$pager->getLastPage()]) ?]
[?php endif; ?]
</div>
