...
class ExportVocab {
     ...
     */
    public function __construct( $vocabId, $userId, $download = false, $asTemplate = false, $languages = array() )
    {
        ...
        if ( empty( $languages ) )
        {
            $this->setLanguages( $this->getSchema()->getLanguages() );
        }
        if ( ! $download )
        {
            $this->setPath(
              SF_ROOT_DIR . DIRECTORY_SEPARATOR . 'web' . DIRECTORY_SEPARATOR . 'repos' . DIRECTORY_SEPARATOR . 'agents' . DIRECTORY_SEPARATOR
    ...
    public function write()
    {
        ...
        if ( ! $this->isTemplate() )
        {
            ...
            $properties = $this->getSchema()->getSchemaPropertys( $c );
    ...
    public function getPrologHeader()
    {
        ...
        $rows[0][1] = '';
        $rows[1][1] = '';
        $rows[2][1] = $this->getSchema()->getLanguage(); //default language
        $rows[3][1] = 'uri'; //default type
    ...
    public function getMetadata()
    {
        ...
        $rows        = array_fill( 0, 10, array_fill( 0, $headerCount, '' ) );
        for ( $I = 0; $I < 10; $I ++ )
        {
            $rows[$I][0] = "meta";
        }
        ...
        $rows[2][1] = 'status_id';
        $rows[2][2] = $this->getSchema()->getStatusId();
        $rows[3][1] = 'uri';
