...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
  ...
  const PEER = 'sfGuardUserPeer';
	...
	/**
	 * @var        array sfBlogLog[] Collection to store aggregation of sfBlogLog objects.
	 */
	protected $collsfBlogLogs;

	/**
	 * @var        Criteria The criteria used to select the current contents of collsfBlogLogs.
	 */
	private $lastsfBlogLogCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collsfBlogLogs = null;
			$this->lastsfBlogLogCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collsfBlogLogs !== null) {
				foreach ($this->collsfBlogLogs as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getsfBlogLogs() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addsfBlogLog($relObj->copy($deepCopy));
				}
			}
	 ...
	 * related sfBlogPosts from storage. If this sfGuardUser is new, it will return
		...
		if ($this->collsfBlogPosts === null) {
			if ($this->isNew()) {
			   $this->collsfBlogPosts = array();
				...
				$criteria->add(sfBlogPostPeer::AUTHOR_ID, $this->id);
	 ...
	 */
	public function getsfBlogPostsJoinsfBlog($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collsfBlogPosts === null) {
			if ($this->isNew()) {
				...
				$this->collsfBlogPosts = sfBlogPostPeer::doSelectJoinsfBlog($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related sfBlogUsers from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collsfBlogUsers === null) {
			if ($this->isNew()) {
			   $this->collsfBlogUsers = array();
				...
				$criteria->add(sfBlogUserPeer::USER_ID, $this->id);
	 ...
	 * actually need in sfGuardUser.
	 */
	public function getsfBlogUsersJoinsfBlog($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collsfBlogUsers === null) {
			if ($this->isNew()) {
				...
				$this->collsfBlogUsers = sfBlogUserPeer::doSelectJoinsfBlog($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	/**
	 * Clears out the collsfBlogLogs collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 ...
	 * @see        addsfBlogLogs()
	 */
	public function clearsfBlogLogs()
	{
		$this->collsfBlogLogs = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collsfBlogLogs collection (array).
	 *
	 * By default this just sets the collsfBlogLogs collection to an empty array (like clearcollsfBlogLogs());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initsfBlogLogs()
	{
		$this->collsfBlogLogs = array();
	}

	/**
	 * Gets an array of sfBlogLog objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related sfBlogLogs from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array sfBlogLog[]
	 * @throws     PropelException
	 */
	public function getsfBlogLogs($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collsfBlogLogs === null) {
			if ($this->isNew()) {
			   $this->collsfBlogLogs = array();
				...
				$criteria->add(sfBlogLogPeer::USER_ID, $this->id);

				sfBlogLogPeer::addSelectColumns($criteria);
				$this->collsfBlogLogs = sfBlogLogPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				sfBlogLogPeer::addSelectColumns($criteria);
				if (!isset($this->lastsfBlogLogCriteria) || !$this->lastsfBlogLogCriteria->equals($criteria)) {
					$this->collsfBlogLogs = sfBlogLogPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastsfBlogLogCriteria = $criteria;
		return $this->collsfBlogLogs;
	}

	/**
	 * Returns the number of related sfBlogLog objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related sfBlogLog objects.
	 * @throws     PropelException
	 */
	public function countsfBlogLogs(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collsfBlogLogs === null) {
			if ($this->isNew()) {
				...
				$count = sfBlogLogPeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastsfBlogLogCriteria) || !$this->lastsfBlogLogCriteria->equals($criteria)) {
					...
					$count = count($this->collsfBlogLogs);
				}
			} else {
				$count = count($this->collsfBlogLogs);
			}
	...
	/**
	 * Method called to associate a sfBlogLog object to this object
	 * through the sfBlogLog foreign key attribute.
	 *
	 * @param      sfBlogLog $l sfBlogLog
	 * @return     void
	 * @throws     PropelException
	 */
	public function addsfBlogLog(sfBlogLog $l)
	{
		if ($this->collsfBlogLogs === null) {
			$this->initsfBlogLogs();
		}
		if (!in_array($l, $this->collsfBlogLogs, true)) { // only add it if the **same** object is not already associated
			array_push($this->collsfBlogLogs, $l);
			$l->setsfGuardUser($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collsfBlogLogs) {
				foreach ((array) $this->collsfBlogLogs as $o) {
					$o->clearAllReferences($deep);
				}
			}
