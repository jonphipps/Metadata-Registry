...
<?php endif; ?>
  <th id="sf_admin_list_th_<?php echo $column->getName() ?>">
    <?php if ($column->isReal()): ?>
      ...
      [?php echo image_tag(sfConfig::get('sf_admin_web_dir').'/images/s_asc.gif', array('align' => 'absmiddle', 'alt' => __('Ascending Order'), 'title' => __('List has been sorted in ascending order'))) ?]
      [?php elseif ($sf_user->getAttribute('type', null, 'sf_admin/<?php echo $this->getSingularName() ?>/sort') == 'desc'): ?]
      [?php echo image_tag(sfConfig::get('sf_admin_web_dir').'/images/s_desc.gif', array('align' => 'absmiddle', 'alt' => __('Descending Order'), 'title' => __('List has been sorted in descending order'))) ?]
      [?php endif; ?]
      [?php else: ?]
